...
**Workspace**:
Contexto de trabalho que define a fronteira lógica de dados, os usuários participantes, a assinatura e o perfil de uso da aplicação. Um usuário pode participar de vários workspaces e ter permissões diferentes em cada um. O workspace possui um tipo explícito: `Pessoal`, `Agencia` ou `Empresa`. Somente o workspace empresarial possui um cadastro formal de `Empresa` associado. O backend permite ao usuário autenticado consultar seus workspaces e selecionar um deles para a sessão atual após validar o pertencimento. `Sessao/Info` lê essa seleção, valida novamente o vínculo e retorna o workspace atual ao Angular, que o apresenta no dashboard e no layout das rotinas sem manter outro identificador no navegador. O contexto selecionado ainda não participa do RBAC ou do isolamento de dados.
