...
namespace PersonalTracking
{
    public partial class FrmPermissionList : Form
    {
        ...
        {
            if (detail.PermissionDayAmount == 0)
                MessageBox.Show("Please seleect a permission from table");
            else if (detail.State == PermissionStates.Approved || detail.State == PermissionStates.Disapproved)
                MessageBox.Show("You can not uppdate any approved or disapproved permission");
            else
            {
                FrmPermission frm = new FrmPermission();
                frm.isUpdate = true;
        ...
        private void FrmPermissionList_Load(object sender, EventArgs e)
        {
            ...
            if(!UserStatic.isAdmin)
            {
                ...
                btnDisApproved.Hide();
                btnDelete.Hide();
                btnClose.Location = new System.Drawing.Point(423, 18);
            }
        ...
        private void dataGridView1_RowEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            detail.UserNo = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells[10].Value);
